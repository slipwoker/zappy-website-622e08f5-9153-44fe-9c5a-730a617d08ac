      ...
      if (!window.zappyContactFormLoaded) {
        ...
        (async function() {
          try {
            ...
            // Extract message field if exists, otherwise create a default message
            const messageField = document.getElementById('message');
            const message = messageField ? messageField.value : `Contact request from ${name}. Phone: ${phone}`;
            
            // Extract subject field if exists
            const subjectField = document.getElementById('subject');
            const subject = subjectField ? subjectField.value : 'Contact Form Submission';
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: '622e08f5-9153-44fe-9c5a-730a617d08ac',
                name: name,
                email: email,
                subject: subject,
                message: message,
            ...
            if (response.ok) {
              ...
              console.log('⚠️ Zappy: API returned non-OK status', response.status, result);
            }
          } catch (error) {
    ...
    "translations": {
      "en": {
        "consentModal": {
          ...
          "description": "Swift Restoration uses cookies to enhance your experience, analyze site usage, and assist in our marketing efforts. You can manage your preferences anytime.",
